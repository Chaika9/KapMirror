...
#include "Core/NetworkConnectionToServer.hpp"
...
namespace KapMirror {
    ...
    class NetworkClient {
      private:
        ...
        KapEngine::Dictionary<ushort, std::shared_ptr<std::function<void(std::shared_ptr<NetworkConnectionToServer>, NetworkReader&)>>>
            handlers;
      ...
      public:
         ...
         */
        bool isActive() const { return connectState == ConnectState::Connecting || connectState == ConnectState::Connected; }

        /**
         * @brief Check if client is connecting (before isConnected).
         */
        bool isConnecting() const { return connectState == ConnectState::Connecting; }

        /**
         * @brief Check if client is connected (after isConnecting).
         */
        bool isConnected() const { return connectState == ConnectState::Connected; }
        ...
        void registerHandler(std::function<void(const std::shared_ptr<NetworkConnectionToServer>&, T&)> handler) {
            ...
            auto messageHandler = [handler](std::shared_ptr<NetworkConnectionToServer> conn, NetworkReader& reader) {
                T message;
                message.deserialize(reader);
                handler(conn, message);
            };
            handlers[msgType] =
                std::make_shared<std::function<void(std::shared_ptr<NetworkConnectionToServer>, NetworkReader&)>>(messageHandler);
        }
